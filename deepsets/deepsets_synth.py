...
def deepsets_invariant_synth(
    ...
    # Phi network.
    x_phi = qkeras.QDense(
        nnodes_phi, kernel_quantizer=nbits, bias_quantizer=nbits)(deepsets_input)
    x_phi = qkeras.QActivation(activ)(x_phi)

    x_phi = qkeras.QDense(
        nnodes_phi, kernel_quantizer=nbits, bias_quantizer=nbits)(x_phi)
    x_phi = qkeras.QActivation(activ)(x_phi)
    ...
    # Rho network.
    x_rho = qkeras.QDense(
        nnodes_rho, kernel_quantizer=nbits, bias_quantizer=nbits)(sum_output)
    x_rho = qkeras.QActivation(activ)(x_rho)

    deepsets_output = qkeras.QDense(
        nclasses, kernel_quantizer=nbits, bias_quantizer=nbits)(x_rho)
...
def deepsets_equivariant_synth(
    ...
    x_lambd = qkeras.QDense(
        nnodes_phi, use_bias=False, kernel_quantizer=nbits, bias_quantizer=nbits)(x_max)
    x_gamma = qkeras.QDense(
        nnodes_phi, kernel_quantizer=nbits, bias_quantizer=nbits)(deepsets_input)
    ...
    x = qkeras.QActivation(activ)(x)
    deepsets_output = qkeras.QDense(
        nclasses, kernel_quantizer=nbits, bias_quantizer=nbits)(x)
